...
const SEGMENTS = ["All Users", "Active (7d)", "Paying Users", "Inactive (30d+)"];
...
export default function BroadcastDrawer() {
  ...
  return (
    <ModalShell open={!!broadcastBotId} onClose={closeBroadcast} title={`Broadcast — ${bot?.name ?? ""}`} wide>
      <form onSubmit={handleSend} className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="font-mono text-xs text-slate-500">
            Estimated reach: <span className="text-cyan-neon">{bot?.activeUsers.toLocaleString()}</span> users
          </p>
