  ...
  return (
    <GlassPanel glow="cyan" className="p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h3 className="text-sm font-bold text-white">گنجینه‌ی کد</h3>
        <div className="relative">
          ...
          <input
            ...
            className="w-48 rounded-full border border-slate-700 bg-void/60 py-1.5 pl-3 pr-8 text-xs text-white placeholder:text-slate-500 focus:border-cyan-neon focus:outline-none"
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {visibleSnippets.map((snippet) => (
          <div key={snippet.id} className="rounded-xl border border-slate-800">
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-2.5">
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-1.5">
                  <span className="rounded bg-purple-neon/10 px-2 py-0.5 text-[10px] text-purple-neon">{snippet.lang}</span>
                  {snippet.tags.map((tag) => (
                    <span key={tag} className="rounded bg-slate-800 px-2 py-0.5 text-[10px] text-slate-400">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <pre dir="ltr" className="overflow-x-auto p-4 text-left font-mono text-xs text-emerald-neon">
              <code>{snippet.code}</code>
            </pre>
          </div>
        ))}
        {visibleSnippets.length === 0 && <p className="py-6 text-center text-sm text-slate-500">کدی با این جستجو پیدا نشد.</p>}
      </div>
    </GlassPanel>
  );
}
