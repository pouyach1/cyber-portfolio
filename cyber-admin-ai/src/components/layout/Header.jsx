...
export default function Header() {
  ...
  const deepFocusUntil = useDashboardStore((s) => s.deepFocusUntil);
  const current = NAV_TABS.find((t) => t.id === activeTab);
  const focusActive = deepFocusUntil && deepFocusUntil > Date.now();
  ...
  return (
    <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-3">
        {focusActive && (
          <span className="flex items-center gap-1.5 rounded-full border border-purple-neon/40 bg-purple-neon/10 px-3 py-1.5 text-xs text-purple-neon">
            <Moon size={13} /> حالت تمرکز عمیق فعال است
          </span>
        )}
